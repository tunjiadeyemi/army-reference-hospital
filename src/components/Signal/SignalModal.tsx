...
import { mockOrderData, mockSignalFormData } from '../../utils/constants';
...
const SignalModal = () => {
  ...
  const { setShowSickModal, selectedSickRecord } = useContext(AppContext);
  ...
  return (
    <Modal>
      <div className="bg-white w-[85%] h-[90vh] rounded-md shadow-md overflow-y-scroll overflow-hidden">
        ...
        {/* body */}
        <SignalForm isEdit={editMode} mockData={mockSignalFormData} />
      </div>
    </Modal>
  );
};
